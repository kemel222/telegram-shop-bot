...
async def choose_payment_method(callback: CallbackQuery, state: FSMContext):
    ...
    async with async_session_maker() as session:
        ...
        # Отправляем пользователю сообщение о связи с менеджером
        manager_message = f"""
📞 *Ваш заказ оформлен!*

Если у вас есть вопросы по заказу #{order.id}, свяжитесь с нашим менеджером:
👉 @{settings.MANAGER_USERNAME}

Менеджер поможет вам с:
• Уточнением деталей заказа
• Изменением заказа
• Консультацией по товарам
• Любыми другими вопросами

Будем рады помочь! 😊
"""
        await callback.message.answer(manager_message, parse_mode="Markdown")
        
        # Уведомляем админов через админ-бота
