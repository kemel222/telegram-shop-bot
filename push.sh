...
echo -e "${BLUE}🚀 Загрузка проекта на GitHub...${NC}"
...
# Добавляем remote origin (если еще не добавлен)
...
echo -e "${YELLOW}⚠️ Вам может потребоваться ввести логин и пароль GitHub${NC}"
echo -e "${YELLOW}   Или настроить Personal Access Token${NC}"
echo ""
...
if git push -u origin main; then
    ...
    echo -e "${YELLOW}   1. Проверьте логин и пароль${NC}"
    echo -e "${YELLOW}   2. Настройте Personal Access Token${NC}"
    echo -e "${YELLOW}   3. Убедитесь, что репозиторий существует на GitHub${NC}"
    echo ""
    echo -e "${BLUE}🔧 Настройка Personal Access Token:${NC}"
    echo -e "${BLUE}   1. Перейдите в GitHub Settings > Developer settings > Personal access tokens${NC}"
    echo -e "${BLUE}   2. Создайте новый token с правами 'repo'${NC}"
    echo -e "${BLUE}   3. Используйте token вместо пароля при push${NC}"
    exit 1
fi
